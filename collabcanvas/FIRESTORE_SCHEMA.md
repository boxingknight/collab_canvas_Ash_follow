...
Each document represents a single shape on the canvas. Shapes can be rectangles, circles, lines, or text.
...
```typescript
{
  ...
  fontWeight: string;      // Font weight: "normal" | "bold" (default: "normal")
  width: number;          // Text box width in pixels
  height: number;         // Text box height in pixels
}
```
...
Text shapes provide rich text editing capabilities:

**Coordinate System:**
- `x, y` = Top-left corner of the text box
- `width, height` = Text box dimensions (auto-resized based on content)

**Text Properties:**
- `text` = The actual text content (default: "Text")
- `fontSize` = Font size in pixels (default: 16)
- `fontWeight` = Font weight: "normal" or "bold" (default: "normal")
- `color` = Text color (hex code)

**Editing Behavior:**
- Double-click text to enter edit mode
- Text editor overlay appears with textarea
- Enter to save, Escape to cancel
- Text box auto-resizes based on content
- Multi-line support with word wrapping

**Locking:**
- Text shapes use the same locking mechanism as other shapes
- Lock icon displays at the top-left corner
- Other users see reduced opacity and cannot edit locked text
- Edit locking prevents simultaneous text editing
